...
```mermaid
---
title: Histology directory schema (Version 2.0)
---
flowchart LR
    root[/"`**.**
    *(root directory in globus)*`"/] --> meta["`**histology_metadata.tsv**`"]
    ...
    *or: … .qptiff, .scn, .czi, 
    .svs, .ndpi, .vsi, .xml*`"]
    ex --> mshw["`**microscope_hardware.json**`"] & mssettings["`microscope_settings.json
    *optional*`"]:::opt
    lpimg --> tb["`… .tissue-boundary.geojson
    *optional*`"]:::opt
    lp --> trans[/"`transformations/
    *optional*`"/]:::opt --> txt["`… .txt
    *optional*`"]:::opt
    lp --> prob[/"`probabilities/
    *optional*`"/]:::opt --> probi["`… .tiff
    *optional*`"]:::opt
    lp --> ann[/"`annotations/
    *optional*`"/]:::opt --> annfiles["`*any / all:*
    … .segmentations.ome.tiff
    segmentation_masks.csv
    … -objects.csv
    … .geojson
