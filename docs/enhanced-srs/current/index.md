...
```mermaid
---
title: Enhanced Stimulated Raman Spectroscopy (SRS) directory schema (Version 2.0)
---
flowchart LR
    root[/"`**.**
    *(root directory in globus)*`"/] --> meta["`**enhanced-srs_metadata.tsv**`"]
    ...
    *optional*`"]:::opt
    ...
    raw --> light["`lightpath_configuration.tsv
    *and / or* fingerprint_layout.tsv
    *optional*`"]:::opt
    raw --> rawi[/"`**images/**`"/]
    rawi --> tiff["`**… .tiff**
